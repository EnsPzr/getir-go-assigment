...
// This test method testing go-cache(in-memory) handlers.
func TestInMemoryGoCache(t *testing.T) {
	// Initialize go-cache service.
	cache.InitGoCache()
	// Setup router.
	...
	for _, tk := range newTest {
		t.Run("Handlers-set-sqlite cache:"+tk.input.Key, func(t *testing.T) {
			...
			resp, err := http.Post("http://localhost:8081/in-memory",
	...
	for _, tk := range newTest {
		t.Run("Handlers-get-sqlite cache:"+tk.input.Key, func(t *testing.T) {
			resp, err := http.Get(fmt.Sprintf("http://localhost:8081/in-memory?key=%s", tk.input.Key))
			if err != nil {
				t.Errorf("Post Request Error => %s", err.Error())
			}
